 ...
 *
 */
public abstract class XAbstractStreamListeningInvoker
	...
	public XAbstractStreamListeningInvoker(FunctionCatalog functionCatalog,
	...
	protected FluxMessageProcessor select(Message<?> input) {
		...
		if (input.getHeaders().containsKey(XStreamConfigurationProperties.ROUTE_KEY)) {
			String key = (String) input.getHeaders()
					.get(XStreamConfigurationProperties.ROUTE_KEY);
			processor = stash(key);
		}
